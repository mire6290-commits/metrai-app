...
# --- 2. EXCEL ENGINE (STYLE IDENTIQUE AU MODÈLE) ---
def generate_excel_pro(df):
    output = io.BytesIO()
    with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
        df.to_excel(writer, index=False, sheet_name='📋 MÉTRÉ_EXPERT', startrow=1)
        workbook, worksheet = writer.book, writer.sheets['📋 MÉTRÉ_EXPERT']
...
# --- 3. UI STYLE MODERNE ---
st.set_page_config(page_title="Metrai AI Expert V7", layout="wide")
...
if st.session_state.step == 'upload':
    st.markdown("<h1 style='text-align:center;'>🏗️ METRAI AI <span style='color:#ed7d31'>EXPERT V7</span></h1>", unsafe_allow_html=True)
    file = st.file_uploader("Charger le PLAN.pdf pour analyse complète", type="pdf")
    if file and st.button("LANCER L'EXTRACTION HAUTE RÉSOLUTION"):
    ...
    for i in range(len(doc)):
        log.info(f"🔍 Analyse de la Page {i+1}/{len(doc)} par Grid-Scanning...")
        page = doc[i]
        # Grid Scanning 2x2 bach n-weslo l'natija s-m-ih
        w, h = page.rect.width, page.rect.height
        grids = [(0,0,w/2,h/2), (w/2,0,w,h/2), (0,h/2,w/2,h), (w/2,h/2,w,h)]
        
        for idx, rect in enumerate(grids):
            pix = page.get_pixmap(matrix=fitz.Matrix(3.5, 3.5), clip=rect)
            b64 = base64.b64encode(pix.tobytes("png")).decode('utf-8')
            
            prompt = """Expert en Charpente : Extrais tous les profilés métalliques de cette zone du plan.
            Profilés cherchés : IPE, HEA, UPN, Cornières L, SQ, RO.
            Si une longueur manque, extrais-la des annotations.
            JSON: {"members": [{"profile": "IPE 600", "quantity": 1, "length_m": 6.0, "type": "Poutre", "location": "Niveau 1"}]}"""
    ...
    if not st.session_state.results:
        ...
        # Nettoyage strict kima l-modèle
        ...
        c3.markdown(f'<div class="metric-box">NB ÉLÉMENTS<br><h2>{int(df["Quantité"].sum())}</h2></div>', unsafe_allow_html=True)
        
        st.write("---")
        btn_a, btn_b = st.columns(2)
        with btn_a: st.download_button("📥 TÉLÉCHARGER LE MÉTRÉ MODÈLE", generate_excel_pro(df), "Metre_Expert_Final.xlsx")
        with btn_b: 
            if st.button("🔄 NOUVELLE EXTRACTION"): st.session_state.step = 'upload'; st.rerun()
            
        st.write("### 📋 DÉTAILS DE L'EXTRACTION")
        # Fix warning 2026: width='stretch'
