...
API_KEY = "tkpq3wSOfVMoWLXee7wP3vHyEgJGm87m"
...
def generate_pro_excel(df, total_kg, total_qty):
    ...
    # On laisse les colonnes intermédiaires vides ou avec format simple
    for c in range(2, 6):
        worksheet.write(last_row, c, "", total_format)
    
    # Colonne Poids Total (index 6 dans le DataFrame généré en step results)
...
with st.sidebar:
    ...
    st.write("---")
    st.markdown('<div style="color:#666; font-size:12px;">Version 2.1.0 Premium<br>© 2026 Metrai Structure</div>', unsafe_allow_html=True)
...
if st.session_state.step == 'upload':
    ...
    with st.container():
        ...
        try:
            ...
            st.error(f"Oups! Kayn mouchkil: {e}")
    ...
    # Génération Excel avec Design Pro et Ligne Total
    xl_data = generate_pro_excel(df, t_kg, t_qty)
    
    col_a, col_b = st.columns(2)
    with col_a:
        st.download_button(
            label="📥 TÉLÉCHARGER LE RAPPORT EXCEL (PRO DESIGN)", 
            data=xl_data, 
            file_name=f"Metrai_Pro_Takeoff_{int(time.time())}.xlsx", 
    ...
    with email_col1:
        target_email = st.text_input("Adresse Email du destinataire", placeholder="exemple@entreprise.com")
    with email_col2:
        if st.button("Sift Daba"):
            if target_email:
                st.toast(f"✅ Résultat envoyé avec succès à {target_email}!")
            else:
                st.error("Veuillez entrer un email.")

st.markdown('<div class="footer-text">Propulsé par Pixtral Vision AI - Développé pour les Ingénieurs Structure</div>', unsafe_allow_html=True)